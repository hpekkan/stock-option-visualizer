...
def main():
    ...
    for i in range(100):
        ...
        stock_profit_val = stock_profit_val + stock_profit(spot_price,option_x[i],stock_info)*stock_quantity
        option_profit[i] = option_profit_val
        stock_profit_x[i] = stock_profit_val

  

    plt.plot(option_x,option_profit, color='green', label='Opsiyon Kar/Zarar')
    plt.plot(option_x,stock_profit_x, color='blue', label='Hisse Senedi Kar/Zarar')
    plt.plot(option_x,option_profit+stock_profit_x, color='red', label='Toplam Kar/Zarar')
