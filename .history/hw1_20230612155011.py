...
def main():
   ...
   # Find intersection points
    ...
    for i in range(len(intersections) - 1):
        ...
        intersection_2 = intersections[i + 1]
        
        if np.sign(np.sum(option_profits[:, intersection_1])) != np.sign(np.sum(option_profits[:, intersection_2])):
            x1 = option_x[intersection_1]
            x2 = option_x[intersection_2]
            
            if x1 < 0 and x2 > 0:
                plt.plot(0, 0, 'o', markersize=5, color='red')
                plt.text(0, 0, '(0, 0)', ha='center', va='bottom', color='purple')
            
            x3 = option_x[intersection_1 + 1]
            x4 = option_x[intersection_2 - 1]
            
            plt.plot(x3, 0, 'o', markersize=5, color='red')
            plt.text(x3, 0, f'({x3:.2f}, 0)', ha='center', va='bottom', color='purple')
            plt.plot(x4, 0, 'o', markersize=5, color='red')
            plt.text(x4, 0, f'({x4:.2f}, 0)', ha='center', va='bottom', color='purple')
    # Find intersection points
    intersections = np.where(np.diff(np.signbit(np.sum(option_profits, axis=0)+ stock_profit_x)) != 0)[0]
    print(np.signbit(np.sum(option_profits, axis=0)+ stock_profit_x))
    # Plot intersection points
    for intersection in intersections:
        plt.plot(option_x[intersection], 0, 'ro', markersize=5)
        plt.text(option_x[intersection], 0, f'({option_x[intersection]:.2f}, 0)', ha='center', va='bottom', color="red")
