    ...
    try:
            ...
            for option in option_data:
                if option["option_type"].casefold() == "put".casefold():
                    if option["option_info"].casefold() == "alım".casefold():
                        profit += long_put(option["strike_price"], option_x[i], option["option_price"])
                    else:
                        profit += short_put(option["strike_price"], option_x[i], option["option_price"])
                else:
                    if option["option_info"].casefold() == "alım".casefold():
                        profit += long_call(option["strike_price"], option_x[i], option["option_price"])
                    else:
                        profit += short_call(option["strike_price"], option_x[i], option["option_price"])

            profit += stock_profit(spot_price, option_x[i], stock_info) * stock_quantity
            option_y[i] = profit
        ...
        for i in range(1, 100):
            if option_y[i] != option_y[i - 1]:
                if option_y[i] > option_y[i - 1]:
                    print("Option arbitrage opportunity: Buy at option price", i - 1, option_x[i - 1],
                          "and sell at spot price", option_x[i])
                else:
                    print("Option arbitrage opportunity: Sell at option price", i - 1, option_x[i - 1],
                          "and buy at spot price", option_x[i])
        ...
        if max_profit != min_profit:
            ...
            if max_profit > min_profit:
                print("Stock arbitrage opportunity: Buy at option price", min_index, option_x[min_index],
                      "and sell at spot price", option_x[max_index])
            else:
                print("Stock arbitrage opportunity: Sell at option price", min_index, option_x[min_index],
                      "and buy at spot price", option_x[max_index])
