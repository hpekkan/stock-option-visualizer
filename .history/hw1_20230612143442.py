...
def print_arbitrage_opportunities(option_data,spot_price):
    ...
    for i in range(num_options):
        for j in range(i+1, num_options):
            option1 = option_data[i]
            option2 = option_data[j]
            option1_type = option1["option_type"]
            option1_info = option1["option_info"]
            option1_strike = option1["strike_price"]
            option1_price = option1["option_price"]

            option2_type = option2["option_type"]
            option2_info = option2["option_info"]
            option2_strike = option2["strike_price"]
            option2_price = option2["option_price"]

            if option1_type.casefold() == option2_type.casefold():
                continue  # Skip if both options have the same type (put or call)

            if option1_info.casefold() != option2_info.casefold():
                continue  # Skip if both options don't have the same buy/sell info

            if option1_type.casefold() == "put".casefold():
                if option1_info.casefold() == "alım".casefold() and option1_price + option1_strike > option2_price + option2_strike:
                    print(f"Arbitrage opportunity detected between Put option {i+1} and Put option {j+1}")
                elif option1_info.casefold() == "satım".casefold() and option1_price + option1_strike > option2_price + option2_strike:
                    print(f"Arbitrage opportunity detected between Put option {i+1} and Put option {j+1}")
            elif option1_type.casefold() == "call".casefold():
                if option1_info.casefold() == "alım".casefold() and option1_price + option1_strike > option2_price + option2_strike:
                    print(f"Arbitrage opportunity detected between Call option {i+1} and Call option {j+1}")
                elif option1_info.casefold() == "satım".casefold() and option1_price + option1_strike > option2_price  + option2_strike:
                    print(f"Arbitrage opportunity detected between Call option {i+1} and Call option {j+1}")
    ...
    for i, option in enumerate(option_data):
        ...
        if option_type.casefold() == "put".casefold():
            if option_info.casefold() == "alım".casefold() and option_price+strike_price >  spot_price:
                print(f"Arbitrage opportunity detected in Long Put option {i+1}")
        elif option_type.casefold() == "call".casefold():
            if option_info.casefold() == "alım".casefold() and option_price +strike_price< spot_price :
                print(f"Arbitrage opportunity detected in Long Call option {i+1}")
...
def main():
    ...
    while counter <= option_quantity:
        ...
        counter = counter + 1
    mean = mean / counter  
    option_x = np.linspace(0, 3.25 * mean, size)
    ...
    # Find intersection points
    intersections = np.where(np.diff(np.signbit(np.sum(option_profits, axis=0)+ stock_profit_x)) != 0)[0]
