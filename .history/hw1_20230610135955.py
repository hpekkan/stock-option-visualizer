...
def main():
    ...
    for i in range(100):
        ...
        if i > 0 and option_y[i] * stock_quantity != option_y[i-1] * stock_quantity:
            if option_y[i] * stock_quantity > option_y[i-1] * stock_quantity:
                print("Stock arbitrage opportunity: Buy at option ", i-1, option_x[i-1], "and sell at spot price", option_x[i])
            else:
                print("Stock arbitrage opportunity: Sell at spot price", i-1, option_x[i-1], "and buy at spot price", option_x[i])

    plt.plot(option_x,option_y)
    plt.axhline(0, color='black', linewidth=.5,markevery=1 )
