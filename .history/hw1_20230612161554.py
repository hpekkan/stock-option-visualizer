...
def print_arbitrage_opportunities(option_data, spot_price):
    for i, option in enumerate(option_data):
        ...
        if option_type.casefold() == "put".casefold():
            if (
                option_info.casefold() == "alım".casefold()
                and option_price + strike_price > spot_price
            ):
                print(
                    f"Put Alım opsiyonunda arbitraj fırsatı tespit edildi. Opsiyon {i+1}"
                )
            ...
            if (
                ...
                print(
                    f"Call Alım opsiyonunda arbitraj fırsatı tespit edildi. Opsiyon {i+1}"
                )
    ...
    for i, option in enumerate(option_data):
        ...
        if option_type.casefold() == "put".casefold():
            if (
                ...
                and option_price
                > max(
                    other_option["option_price"]
                    for other_option in option_data
                    if other_option["option_type"].casefold() == "put".casefold()
                    and other_option["option_info"].casefold() == "alım".casefold()
                    and other_option["strike_price"] < strike_price
                )
            ):
                print(
                    f"Put Alım opsiyonunda arbitraj fırsatı tespit edildi. Opsiyon {i+1}"
            ...
            if (
                ...
                > max(
                    other_option["option_price"]
                    for other_option in option_data
                    if other_option["option_type"].casefold() == "call".casefold()
                    and other_option["option_info"].casefold() == "alım".casefold()
                    and other_option["strike_price"] > strike_price
                )
            ):
...
def main():
    ...
    option_profits = np.zeros(
        (option_quantity, size)
    )  # Modify to store separate option profits
    counter = 1
    ...
    for i in range(size):
        stock_profit_val = 0
        # Iterate over each option separately
        for j, option in enumerate(option_data):
            ...
            if option["option_type"].casefold() == "put".casefold():
                if option["option_info"].casefold() == "alım".casefold():
                    option_profit_val = (
                        long_put(
                            option_x[i], option["strike_price"], option["option_price"]
                        )
                        * stock_quantity
                    )
                else:
            ...
            option_profits[j][
                i
            ] = option_profit_val  # Store the option profit for each option

        stock_profit_val = (
            stock_profit(option_x[i], spot_price, stock_info) * stock_quantity
        )
        stock_profit_x[i] = stock_profit_val

    print_arbitrage_opportunities(option_data, spot_price)
    spot_price_index = np.where(option_x == spot_price)
    if stock_profit_x[spot_price_index] > 0:
        print(spot_price_index)
    ...
    for k in range(option_quantity):
        plt.plot(
            ...
            option_profits[k],
            color="black",
            label=f"Opsiyon {k+1} Kar",
            linestyle="dashed",
            dashes=(3, 3),
        )  # Plot each option profit separately
    ...
    for counter in range(len(sums) - 1):
        if is_zero != (sums[counter] == 0):
            ...
            is_zero = not is_zero
    intersections = np.where(np.diff(np.signbit(np.sum(option_profits, axis=0))) != 0)[
        0
    ]
    ...
    plt.plot(
        ...
        stock_profit_x,
        color="blue",
        label="Hisse Senedi Kar",
        linestyle="dashed",
        dashes=(3, 3),
    )
    plt.plot(
        option_x, np.sum(option_profits, axis=0), color="purple", label="Opsiyon Karı"
    )
    plt.plot(
        option_x,
