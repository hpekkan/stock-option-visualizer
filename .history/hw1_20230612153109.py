...
def print_arbitrage_opportunities(option_data, spot_price):
    ...
    for i, option in enumerate(option_data):
        ...
        if option_type.casefold() == "put".casefold():
            if option_info.casefold() == "alım".casefold() and option_price + strike_price > spot_price:
                print(f"Put Alım opsiyonunda arbitraj fırsatı tespit edildi. Opsiyon {i+1}")
            put_options.append((strike_price, option_price))
        elif option_type.casefold() == "call".casefold():
            if option_info.casefold() == "alım".casefold() and option_price + strike_price < spot_price:
                print(f"Call Alım opsiyonunda arbitraj fırsatı tespit edildi. Opsiyon {i+1}")
            call_options.append((strike_price, option_price))

    call_options = sorted(call_options, key=lambda x: x[0])
    put_options = sorted(put_options, key=lambda x: x[0], reverse=True)

    for i in range(len(call_options) - 1):
        if call_options[i][0] > call_options[i + 1][0] and call_options[i][1] <= call_options[i + 1][1]:
            print(f"Strike price is lower in Call option, but option price is not larger. Options {i+1} and {i+2}")

    for i in range(len(put_options) - 1):
        if put_options[i][0] < put_options[i + 1][0] and put_options[i][1] >= put_options[i + 1][1]:
            print(f"Strike price is higher in Put option, but option price is not larger. Options {i+1} and {i+2}")

    for call in call_options:
        for put in put_options:
            if call[0] < put[0] and call[1] >= put[1]:
                print(f"Strike price is lower in Call option, but option price is not larger. Call Option: {call}, Put Option: {put}")

    for put in put_options:
        for call in call_options:
            if put[0] > call[0] and put[1] <= call[1]:
                print(f"Strike price is higher in Put option, but option price is not larger. Put Option: {put}, Call Option: {call}")
def main():
