        ...
        for j, option in enumerate(option_data):  # Iterate over each option separately
            ...
            if option["option_type"].casefold() == "put".casefold():
                if option["option_info"].casefold() == "alım".casefold():
                    option_profit_val = long_put(option["strike_price"], option_x[i], option["option_price"]) * stock_quantity
                else:
                    option_profit_val = short_put(option["strike_price"], option_x[i], option["option_price"]) * stock_quantity
            else:
                if option["option_info"].casefold() == "alım".casefold():
                    option_profit_val = long_call(option["strike_price"], option_x[i], option["option_price"]) * stock_quantity
                else:
                    option_profit_val = short_call(option["strike_price"], option_x[i], option["option_price"]) * stock_quantity
