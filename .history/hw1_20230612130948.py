...
def main():
    ...
    intersections = np.argwhere(np.diff(np.signbit(np.sum(option_profits, axis=0) + stock_profit_x))).flatten()

    # Display intersection coordinates as text below the x-axis
    for intersection in intersections:
        x = option_x[intersection]
        plt.text(x, 0, f'({x:.2f}, 0)', ha='center', va='top')

        # Add lines connecting the coordinate to the corresponding line
        y = 0
        for k in range(option_quantity):
            plt.plot([x, x], [y, option_profits[k][intersection]], color='black', linestyle='dotted', alpha=0.5)
            plt.plot([option_x[0], x], [option_profits[k][intersection], option_profits[k][intersection]],
                     color='black', linestyle='dotted', alpha=0.5)

        plt.plot([x, x], [y, stock_profit_x[intersection]], color='black', linestyle='dotted', alpha=0.5)
        plt.plot([option_x[0], x], [stock_profit_x[intersection], stock_profit_x[intersection]],
                 color='black', linestyle='dotted', alpha=0.5)
