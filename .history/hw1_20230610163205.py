...
def main():
    ...
    while counter <= option_quantity:
        ...
        counter = counter + 1
    mean = mean / counter  
    option_x = np.linspace(0, 3 * mean, 100)
    for i in range(100):
        stock_profit_val = 0
    ...
    for k in range(option_quantity):
        plt.plot(option_x, option_profits[k], label=f'Opsiyon {k+1} Kar/Zarar', marker='.',linewidth=0)  # Plot each option profit separately
    if stock_quantity > 0:
        plt.plot(option_x, stock_profit_x, color='blue', label='Hisse Senedi Kar/Zarar',marker=".")
