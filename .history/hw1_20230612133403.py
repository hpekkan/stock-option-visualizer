...
def find_arbitrage(option_x, option_profits, stock_profit_x):
    # Calculate the total profit by summing option profits and stock profit
    total_profits = np.sum(option_profits, axis=0) + stock_profit_x

    # Find the intersection points where the total profit exceeds zero
    intersections = np.where(np.diff(np.signbit(total_profits)) != 0)[0]
    arbitrage_points = []

    for intersection in intersections:
        if total_profits[intersection] > 0:
            arbitrage_points.append((option_x[intersection], total_profits[intersection]))

    return arbitrage_points
def main():
    ...
    arbitrage_points = find_arbitrage(option_x, option_profits, stock_profit_x)

    # Print arbitrage opportunities
    if len(arbitrage_points) > 0:
        print("Arbitrage Opportunities:")
        for point in arbitrage_points:
            print(f"Streak Fiyatı: {point[0]:.2f}, Kar: {point[1]:.2f}")
    else:
