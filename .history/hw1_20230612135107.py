...
def print_arbitrage_opportunities(option_data,spot_price):
    for i, option in enumerate(option_data):
        ...
        if option_type.casefold() == "put".casefold():
            ...
            elif option_info.casefold() == "satım".casefold() and option_price > spot_price - strike_price:
                print(f"Arbitrage opportunity detected in Short Put option {i+1}")
        elif option_type.casefold() == "call".casefold():
            if option_info.casefold() == "alım".casefold() and option_price > spot_price - strike_price:
                print(f"Arbitrage opportunity detected in Long Call option {i+1}")
            elif option_info.casefold() == "satım".casefold() and option_price > strike_price - spot_price:
                print(f"Arbitrage opportunity detected in Short Call option {i+1}")
