...
def main():
    ...
    while counter <= option_quantity:
        ...
        counter = counter + 1
    mean = mean / counter  
    option_x = np.linspace(0, 4 * mean, 100)
    for i in range(100):
        stock_profit_val = 0
            ...
            if option["option_type"].casefold() == "put".casefold():
                if option["option_info"].casefold() == "alım".casefold():
                    if stock_quantity > 0:
                        ...
                        option_profit_val = long_put(option_x[i],option["strike_price"], option["option_price"])
                else:
                    if stock_quantity > 0:
                        option_profit_val = short_put(option_x[i],option["strike_price"], option["option_price"]) * stock_quantity
                    else:
                        option_profit_val = short_put(option_x[i],option["strike_price"], option["option_price"])
                ...
                if option["option_info"].casefold() == "alım".casefold():
                    if stock_quantity > 0:
                        ...
                        option_profit_val = long_call(option_x[i],option["strike_price"],  option["option_price"]) 
                else:
                    if stock_quantity > 0:
                        option_profit_val = short_call(option_x[i],option["strike_price"],  option["option_price"]) * stock_quantity
                    else:
                      option_profit_val = short_call(option_x[i],option["strike_price"],  option["option_price"]) 
    ...
    for k in range(option_quantity):
        plt.plot(option_x, option_profits[k], label=f'Opsiyon {k+1} Kar/Zarar', marker=(5, 2),linewidth=0)  # Plot each option profit separately
    if stock_quantity > 0:
        plt.plot(option_x, stock_profit_x, color='blue', label='Hisse Senedi Kar/Zarar',marker=(5, 2))
    
    plt.plot(option_x, np.sum(option_profits, axis=0)+stock_profit_x, color='red', label='Toplam Kar/Zarar')
